...
export default function App() {
  ...
  const chooseTarget = async () => {
    ...
    const selected = await open(kind === "folder" ? { directory: true, multiple: false } : { directory: false, multiple: false });
    if (selected) setTarget(selected as string);
  };
  ...
  const apply = async () => {
    if (!target.trim()) return showError(kind === "extension" ? "请输入扩展名，例如 .md" : "请先选择目标");
    setBusy(true);
    try {
      const pngData = canvasRef.current!.toDataURL("image/png").split(",")[1];
      if (!isTauri()) throw new Error("当前是浏览器预览模式，请使用桌面应用应用图标");
      await invoke<IconRule>("apply_icon", { target: target.trim(), kind, pngData, name: displayName, addToMonitor: true, recursiveMode: kind === "folder" ? recursiveMode : "none", maxDepth });
      setSettings(await invoke<AppSettings>("load_settings"));
      setNotice("图标已应用并自动创建保护规则");
    } catch (e) { showError(e); } finally { setBusy(false); }
  };
  const displayName = useMemo(() => kind === "extension" ? `${target || ".ext"} 文件` : (target.split(/[\\/]/).pop() || (kind === "folder" ? "新目录" : "文件")), [target, kind]);
...
<span>选择目标类型与位置</span>
            ...
            <div className="segmented" style={{gridTemplateColumns:`repeat(${kinds.length},1fr)`}}>{kinds.map(v => <button key={v} className={kind === v ? "active" : ""} onClick={() => {setKind(v);setTarget("")}}>{v === "folder" ? "目录" : v === "extension" ? "文件类型" : "单个文件"}</button>)}</div>
            <div className="path-row">
<input value={target} onChange={e => setTarget(e.target.value)} placeholder={kind === "extension" ? "输入扩展名，如 .md" : `选择${kind === "folder" ? "目录" : "文件"}路径`}/>{kind !== "extension" && <button onClick={chooseTarget}>
<Folder/>选择</button>}</div>
            {kind === "file" && <div className="hint">macOS 支持单文件图标；Windows 将提示改用扩展名规则。</div>}
